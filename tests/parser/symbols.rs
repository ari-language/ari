...
fn symbol() {
    assert_eq!(
        ...
        (
            Some(Ast::try_from_exprs([Expr::symbol([], 0..6, "symbol")]).ast),
            vec![],
        )
    );
}
    ...
    let (left, right): (Vec<_>, Vec<_>) = builtin_sexpr_names
        ...
        .map(|symbol| {
            (
                parser().parse_recovery(symbol),
                (
                    Some(Ast::try_from_exprs([Expr::symbol([], 0..symbol.len(), symbol)]).ast),
                    vec![],
                ),
            )
        })
...
fn supports_almost_all_of_unicode_with_exceptions() {
    assert_eq!(
        ...
        (
            Some(Ast::try_from_exprs([Expr::symbol([], 0..1, "🙃")]).ast),
            vec![]
        )
    );
}
